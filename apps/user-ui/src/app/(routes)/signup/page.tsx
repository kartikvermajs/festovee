...
//         SignUp
//       </h1>
//       <p className="font-medium text-center text-lg py-3 text-[#00000099]">
//         Home . Signup
//       </p>
...
//             <span className="px-3 cursor-pointer">or Sign in with Email</span>
//             <div className="flex-1 border-t border-gray-300" />
//           </div>
...
const Signup = () => {
  ...
  const signupMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      console.log("signup success", variables);
    ...
    onError: (err) => {
      ...
      if (err && (err as AxiosError).response) {
        alert(
          ((err as AxiosError).response as any).data?.message ?? "Signup failed"
        );
      } else {
  ...
  const verifyOtpMutation = useMutation({
    mutationFn: async () => {
      ...
      const url = `${baseUrl}/api/verify-user`;
    ...
    onError: (err) => {
      ...
      if (err && (err as AxiosError).response) {
        alert(
          ((err as AxiosError).response as any).data?.message ??
            "Verification failed"
        );
      } else {
  ...
  const resendOtp = async () => {
    if (!userData) {
      alert("No user data to resend OTP for");
      return;
    }
    try {
      const url = `${baseUrl}/api/user-registration`;
        ...
        <div className="md:w-[480px] p-8 bg-white shadow rounded-lg">
          ...
          {!showOtp ? (
            ...
            <div>
              ...
              <button
                className="w-full mt-4 text-lg cursor-pointer bg-blue-500 text-white py-2 rounded-lg disabled:opacity-60"
