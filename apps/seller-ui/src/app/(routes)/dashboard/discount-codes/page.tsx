...
const page = () => {
  ...
  const createDiscountCodeMutation = useMutation({
    mutationFn: async (data) => {
      await sellerAxiosInstance.post("/product/api/create-discount-code", data);
    },
    onSuccess: () => {
  ...
  const deleteDiscountCodeMutation = useMutation({
    mutationFn: async (discountId) => {
      await sellerAxiosInstance.delete(
  ...
  return (
    <div className="w-full min-h-screen p-8">
      ...
      <div className="flex items-center text-white">
        <Link
          href={"/dashboard"}
          className="text-[#80deea] cursor-pointer font-Poppins"
        >
          Dashboard
        </Link>
        <ChevronRight size={20} className="opacity-[0.8]" />
        <span className="font-Poppins">Discount Coupons</span>
      </div>
      ...
      <div className="mt-8 bg-gray-900 p-6 rounded-lg shadow-lg">
        ...
        {!isLoading && discountCodes?.length === 0 && (
          <p className="font-Poppins text-gray-400 w-full pt-4 block text-center">
              ...
              <div className="mt-4">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <select
                      {...field}
                      className="mt-1 w-full border outline-none border-gray-700 bg-gray-800 rounded-md p-2 text-white appearance-none focus:border-blue-500"
                    >
                      <option
                        value="percentage"
